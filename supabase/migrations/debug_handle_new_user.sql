-- DEBUG: Catch-all handle_new_user
-- Validates coach_id, safe casts dates.
-- WRAPS INSERT in EXCEPTION block to prevents 500s.
-- If insert fails, user is created without profile (Debugging Only).
...
BEGIN
  ...
  IF v_meta_coach_id IS NOT NULL AND v_meta_coach_id != '' THEN
      BEGIN
          ...
          -- Check existence (skip if not found)
  ...
  -- 5. Insert with EXCEPTION HANDLING
  BEGIN
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = NOW();
   EXCEPTION WHEN OTHERS THEN
       -- LOG error to warning, return NEW so Auth succeeds (but profile fails)
